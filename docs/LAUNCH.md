...
4. `npx airlock-cli init all` showing Claude/Codex/Gemini/Cursor support
...
Airlock sits in the loop: CLI, MCP server, Claude Code hook, and universal shell
shims. It blocks nonexistent npm/PyPI packages, typosquat/mashup names, npx/dlx
package execution, and destructive shell commands before they run.
...
4. Show `init claude-code`, `init codex`, `init gemini`, `init cursor`.
5. End with `npx airlock-cli demo`.
